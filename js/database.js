 ...
 * 获取所有产品的库存历史记录（按店铺）
 * @param {string} storeId - 店铺ID
 * @returns {Promise} 返回所有产品的库存历史记录
 */
function getAllStockHistory(storeId) {
    return new Promise((resolve, reject) => {
        database.ref(`stock_history/${storeId}`)
            .once('value')
            .then(snapshot => {
            ...
            .catch(error => {
                console.error('获取所有库存历史失败:', error);
                reject(error);
            });
    });
}
 ...
 * @returns {Promise} 返回多个店铺的库存历史记录
 */
function getMultiStoreStockHistory(storeIds) {
    return new Promise((resolve, reject) => {
        const promises = storeIds.map(storeId => getAllStockHistory(storeId));
            ...
            .then(results => {
                ...
                results.forEach((history, index) => {
                    if (history && Object.keys(history).length > 0) {
                        combinedHistory[storeIds[index]] = history;
                    }
                });
                resolve(combinedHistory);
            })
            .catch(error => {
                console.error('获取多店铺库存历史失败:', error);
                reject(error);
            });
    });
}
 ...
 * @returns {Promise} 返回过滤后的库存历史记录
 */
function getFilteredStockHistory(storeId, filters = {}) {
    return new Promise((resolve, reject) => {
        getAllStockHistory(storeId)
            .then(allHistory => {
                let filteredHistory = [];
                
                // 遍历所有产品的历史记录
                Object.keys(allHistory).forEach(productId => {
                ...
                if (filters.productId && filters.productId !== 'all') {
                    filteredHistory = filteredHistory.filter(record => record.productId === filters.productId);
                }
            ...
            .catch(error => {
                console.error('获取过滤库存历史失败:', error);
                reject(error);
            });
    });
} 